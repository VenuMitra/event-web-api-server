    ...
    it('responds with home page', function (done) {
        ...
        request(app)
            ...
            .end((err, res) => {
                ...
                chai.assert.isTrue(res.text.includes("Welcome to"));
                return done();
            });
    });
...
describe('POST /event', function () {
    it('adds an event', function (done) {
        const data = { type: "add", title: 'test event', description: 'even cooler test', id: 4321, location: 'Some Test Place', likes: 0 };
        //specify the url to be intercepted
        nock("http://localhost:8082")
            //define the method to be intercepted
            .post('/api/event')
